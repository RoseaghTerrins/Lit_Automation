import pandas as pd
from sqlalchemy import create_engine, MetaData, Table, String, Column, DateTime, Numeric, Float, CheckConstraint, \
    Integer
import sqlalchemy
...
def import_nb_file_to_db(file):
    ...
    data = pd.read_csv(file, skip_blank_lines=False, encoding='gbk')
    df = pd.DataFrame(data)
    df[["DateofBirth", "DefaultDate", "LastPaymentDate", "SupplyDateFrom", "SupplyDateTo", "FinalBillIssueDate",
        "FinalBillFromDate", "FinalBillToDate", "VacatedDate"]] = df[
        ["DateofBirth", "DefaultDate", "LastPaymentDate", "SupplyDateFrom", "SupplyDateTo", "FinalBillIssueDate",
         "FinalBillFromDate", "FinalBillToDate", "VacatedDate"]].apply(pd.to_datetime)
    df[["Telephone1", "Telephone2", "Telephone3", "MPANNumber", "MPRNNumber"]] = df[
        ["Telephone1", "Telephone2", "Telephone3", "MPANNumber", "MPRNNumber"]].astype(str)
    ...
    df[["Telephone1", "Telephone2", "Telephone3", "MPANNumber", "MPRNNumber"]] = df[
        ["Telephone1", "Telephone2", "Telephone3", "MPANNumber", "MPRNNumber"]].replace('0nan', 'NULL')
    df = df.dropna(how='all')
    df.to_sql(name, con=engine, index=False, if_exists='append')


def import_TU_file_to_db(file):
    ...
    data = pd.read_csv(file, skip_blank_lines=False, encoding='gbk')
    df = pd.DataFrame(data)
    df = df[['ClientId', 'DataDNA', 'AddressType', 'MatchLevel', 'P2PScore']]
    df = df.dropna(how='all')
    df.to_sql(f'Lit_{now}_FROM_TU', con=engine, index=False, if_exists='append')


def import_file_to_CS_db(file):
    Server = 'Just-Win10VM\SQLEXPRESS'
    Database = 'RM_Files'
    Driver = 'ODBC Driver 17 for SQL Server'
    Database_Con = f'mssql://@{Server}/{Database}?driver={Driver}'
    ...
    df = df[['ClientId',
             ...
             'DC_SCORE_DATE',
             'DC_CREDIT_SCORE',
             'DC_CREDIT_SCORE_DESC',
             'DC_INTERNATIONAL_SCORE',
             'DC_CREDIT_SCORE_PREVIOUS',
             'DC_CREDIT_SCORE_DESC_PREVIOUS',
             'DC_SCORE_DATE_PREVIOUS',
             'DC_LTD_FLAG',
             'DC_SAFE_NUMBER',
             'DC_ALGORITHM',
             'DC_ALGORITHM_BANDING',
             'DC_ALGORITHM_DESCRIPTION']]
    df = df.dropna(how='all')
    df.to_sql(name, con=engine, index=False, if_exists='append')
    ...
    for item in directory_list:
        try:
            ...
            if 'Activity' and '1st_Locate' in item:
                ...
                newest = max(glob.glob(f'{item}\\*.zip'), key=os.path.getmtime)
                with zipfile.ZipFile(newest, "r") as zip_ref:
                     zip_ref.extractall(settings.FROM_TRANSUNION)
                shutil.move(newest, f'{settings.FROM_TRANSUNION}\\Archive\\')
                newest_file = max(glob.glob(f'{item}\\*_CTOutput.csv'), key=os.path.getmtime)
                import_TU_file_to_db(newest_file)
                shutil.move(newest_file, item + f'\\Processed\\Lit_{now}_FROM_TU.csv')
                csv_recon = max(glob.glob(f'{item}\\*_CTRecon.csv'), key=os.path.getmtime)
                pdf_report = max(glob.glob(f'{item}\\*_Report.pdf'), key=os.path.getmtime)
                os.remove(csv_recon)
                ...
                newest = max(glob.glob(f'{item}\\*.xls'), key=os.path.getmtime)
                wb = open_workbook(newest, encoding_override='latin1')
                df = pd.read_excel(wb)
                os.remove(newest)
                name = f'Lit_{now}_FROM_CS.csv'
                path = item + '\\' + name
                df.to_csv(item + '\\' + name, index=False)
                import_file_to_CS_db(item + '\\' + name)
                shutil.move(path, item + '\\Processed\\')
